...
def test_storage_module_delete_context_def_raises():
    sm = StorageModule()
    with pytest.raises(TypeError):
        asyncio.run(sm.delete_context_def(context_type="x", config={}))
...
def test_storage_module_get_identity_def_raises():
    sm = StorageModule()
    with pytest.raises(TypeError):
        asyncio.run(sm.get_identity_def(identity_type="x", config={}))
...
def test_storage_module_delete_identity_def_raises():
    sm = StorageModule()
    with pytest.raises(TypeError):
        asyncio.run(sm.delete_identity_def(identity_type="x", config={}))
...
def test_storage_module_get_resource_def_raises():
    sm = StorageModule()
    with pytest.raises(TypeError):
        asyncio.run(sm.get_resource_def(resource_type="x", config={}))
...
def test_storage_module_delete_resource_def_raises():
    sm = StorageModule()
    with pytest.raises(TypeError):
        asyncio.run(sm.delete_resource_def(resource_type="x", config={}))
...
def test_storage_module_delete_latch_raises():
    sm = StorageModule()
    with pytest.raises(TypeError):
        asyncio.run(sm.delete_latch(storage_latch_uuid="x", config={}))


def test_storage_module_cleanup_latches_raises():
    sm = StorageModule()
    with pytest.raises(TypeError):
        asyncio.run(sm.cleanup_latches(before=datetime.datetime.now(), config={}))
...
def test_dict_storage_put_and_get_resource_def(storage):
    ...
    asyncio.run(storage.put_resource_def(resource_def, config={}))
...
def test_dict_storage_enact_and_get_grant(storage, sample_grant):
    ...
    result = asyncio.run(storage.get_grant(sample_grant["grant_uuid"], config={}))
    assert result["has_failed"] is False
    assert result["grant"] == sample_grant


def test_dict_storage_get_grant_not_found(storage):
    result = asyncio.run(storage.get_grant("nonexistent-uuid", config={}))
    assert result["has_failed"] is True
...
def test_dict_storage_repeal(storage, sample_grant):
    ...
    assert result["has_failed"] is False
    get_result = asyncio.run(storage.get_grant(sample_grant["grant_uuid"], config={}))
...
def test_dict_storage_create_and_get_latch(storage):
    ...
    get_result = asyncio.run(storage.get_latch(latch["storage_latch_uuid"], config={}))
...
def test_dict_storage_cleanup_latches(storage):
    ...
    asyncio.run(storage.create_latch(config={}))
    future = datetime.datetime.now(tz=datetime.timezone.utc) + datetime.timedelta(seconds=1)
    result = asyncio.run(storage.cleanup_latches(before=future, config={}))
    assert result["has_failed"] is False
...
def test_dict_storage_cleanup_latches_keeps_recent(storage):
    asyncio.run(storage.create_latch(config={}))
    past = datetime.datetime.now(tz=datetime.timezone.utc) - datetime.timedelta(seconds=10)
