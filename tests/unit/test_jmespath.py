...
def test_jmespath_execute_simple_expression():
    result = jmespath_execute("a", {"a": 1, "b": 2})
    assert result == {
        "result": 1,
        "has_failed": False,
        "error_message": None,
...
def test_jmespath_custom_execute_simple():
    result = jmespath_custom_execute("a", {"a": "hello"})
    assert result == {
        "result": "hello",
        "has_failed": False,
        "error_message": None,
...
def test_jmespath_custom_execute_invalid_expression():
    result = jmespath_custom_execute("a.[invalid", {"a": 1})
    assert result["has_failed"] is True
    assert result["result"] is None
    assert "JMESPath Query error" in result["error_message"]
...
def test_inner_join_basic():
    ...
    result = jmespath_custom_execute(
        "inner_join(lhs_arr, rhs_arr, 'lhs == rhs')", data
    )
    assert result["has_failed"] is False
    joined = result["result"]
    assert len(joined) == 2
    assert {"lhs": 2, "rhs": 2} in joined
    assert {"lhs": 3, "rhs": 3} in joined
...
def test_left_join_basic():
    ...
    result = jmespath_custom_execute(
        "left_join(lhs_arr, rhs_arr, 'lhs == rhs')", data
    )
    assert result["has_failed"] is False
    joined = result["result"]
    assert {"lhs": 1, "rhs": None} in joined
    assert {"lhs": 2, "rhs": 2} in joined
    assert {"lhs": 3, "rhs": 3} in joined
...
def test_left_join_no_rhs_matches():
    ...
    joined = result["result"]
    assert {"lhs": 1, "rhs": None} in joined
    assert {"lhs": 2, "rhs": None} in joined
...
def test_outer_join_basic():
    ...
    result = jmespath_custom_execute(
        "outer_join(lhs_arr, rhs_arr, 'lhs == rhs')", data
    )
    assert result["has_failed"] is False
    joined = result["result"]
    assert {"lhs": 1, "rhs": None} in joined
    assert {"lhs": 2, "rhs": 2} in joined
    assert {"lhs": None, "rhs": 3} in joined


def test_regex_find_direct_string_match():
    result = CustomJMESPathFunctions._func_regex_find("\\d+", "hello 123 world")
    assert result == "123"


def test_regex_find_direct_string_no_match():
    result = CustomJMESPathFunctions._func_regex_find("\\d+", "hello world")
...
def test_regex_find_all_direct_string():
    result = CustomJMESPathFunctions._func_regex_find_all("\\d", "a1b2c3")
    assert result == [
        "1",
        "2",
        "3",
...
def test_regex_groups_direct_string_no_match():
    result = CustomJMESPathFunctions._func_regex_groups("(\\d{4})-(\\d{2})", "no date")
    assert result is None
...
def test_regex_groups_all_direct_string():
    result = CustomJMESPathFunctions._func_regex_groups_all("([a-z])(\\d)", "a1b2c3")
    assert result == [
        [
            "a",
            "1",
...
def test_is_identity_present_true():
    ...
    result = jmespath_custom_execute("is_identity_present(itype, request)", data)
