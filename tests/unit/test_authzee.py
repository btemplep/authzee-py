...
def test_validate_grant_with_config(authz, grant):
    result = authz.validate_grant(grant, config={
            "authzee": {
                "raise_crits": True,
            },
        })
...
def test_enact_with_config(authz, grant):
    result = authz.enact(grant, config={
            "authzee": {
                "raise_crits": False,
            },
        })
...
def test_batch_audit_paginator(seeded_authz, batch_request):
    all_grants = []
    all_batch = []
    for page in paginator(seeded_authz.batch_audit, batch_request=batch_request):
        all_grants.extend(page["grants"])
